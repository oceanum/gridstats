...
Derived variables are computed quantities that are added to the dataset *before* any stat function runs. They transform raw dataset variables (e.g. u/v wind components) into derived quantities (e.g. wind speed, direction) that can be used as inputs to any stat.
...
When the derived function name and output variable name are the same and all input variable defaults apply, a bare string shorthand can be used:
...
Multiple derived variables can be stacked in a single call:
...
Returns wind speed (m/s) computed as √(u² + v²).
...
Returns wind **coming-from** direction (degrees, meteorological convention).
...
Returns sea water **going-to** direction (degrees, oceanographic convention).
...
Returns the Douglas sea scale (0–9) from wind-sea Hs. Scale 0 = glassy; scale 9 = phenomenal (Hs > 14 m).
...
Returns the Douglas swell scale (0–9) from swell Hs and wavelength.
...
## `uorb` — Wave orbital velocity {#uorb}

Significant horizontal wave orbital velocity at any height above the seabed, computed from integrated spectral parameters using the monochromatic peak-period approximation (Soulsby 1997).
